...
[[ -z "${DISCORD_BOT_TOKEN// }" ]] && __error "Environment Variable 'DISCORD_BOT_TOKEN' missing or is empty";
[[ -z "${VCB_DB_PATH// }" ]] && __error "Environment Variable 'VCB_DB_PATH' missing or is empty";
...
DC_INFO=$(docker ps --all --format "table {{.Status}}\t{{.Names}}" | awk '/voice-create-bot-docker$/ {print $0}');
...
docker run -d \
	...
	-e VCB_DB_PATH="${VCB_DB_PATH}" \
	-v /mnt/data/${BUILD_PROJECT}/data:/data \
	-t ${DOCKER_IMAGE};
